defmodule SocialScribe.SalesforceSuggestions do
  ...
  @doc """
  Retrieves a field value from a contact map.
  Handles Salesforce field name to atom conversion.
  """
  def get_contact_field(contact, field) when is_map(contact) do
    # Salesforce returns fields as atoms (e.g., :firstname, :lastname)
    # Convert string field name to atom for map access
    # Salesforce internal fields use lowercase atoms (firstname, lastname, etc.)
    # Special case for Company which maps to :company in formatted contact
    field_atom =
      case field do
        "Company" ->
          :company
